...
Here is the Stan code equivalent to the original JAGS code in BMHE.
...
data {
  ...
  real<lower=0> k;  // uniform prior bounds
  real y[N];
  real X[N];
}
...
The R code for running this Stan code and presenting the results is very similar to the original.
...
The plotting code here is simpler than the original JAGS code because we call the plotting functions from `rstan` directly.
